...
const MenuWrapper = ({ className }) => {
  ...
  return (
    <div
      className={`container flex-row align-items-center d-flex ${className}`}
    >
      {user ? (
        <>
          <img
          ...
          >
            <div
              onMouseEnter={onEnterDescription}
              onMouseLeave={onLeaveDescription}
              onClick={handleClick}
              className="d-flex container-fluid ps-2 flex-column justify-content-center description"
            >
      ...
      <div className="buttons ms-auto d-inline-flex">
        <OverlayTrigger
          placement="top"
          overlay={BasicToolTip("Friends")}
          delay={{ show: 0, hide: 0 }}
        >
          <button
