...
const ChatOutWrapper = ({className}) => {
    ...
    const handleTextAreaInput = (e) => {
        ...
        document.documentElement.style.setProperty('--chat-out-height', `${60 + Math.min(len, 4) * 20}px`);
    }
    ...
    return (
        <div className={`${className} chat-out d-flex align-items-center justify-content-center`}>
            <textarea type="search" onInput={handleTextAreaInput} id="chat__content" onKeyDown={handleTextAreaKeydown}/>
            <Picker set="google" theme="dark" onClick={onEmojiClick} style={
                {
                    position: "absolute",
            ...
            <button id="emoji__trigger" onClick={triggerEmojiPicker}><i className="fas fa-smile-wink"></i></button>
        </div>
    )
}
